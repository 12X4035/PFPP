...
std::vector< std::vector<double> > prm_M(int N, double pi, std::vector< std::vector<double> > prm_theta, std::vector< std::vector<double> > up_r, std::vector< std::vector<double> > down_r, std::vector< std::vector<double> > prob) {
    ...
    for(int i = 0;i <= N; i++){
        prm_M_res[i].resize(std::pow(2, i));
        for(int j = 0; j < std::pow(2, i); j++){
...
std::vector< std::vector<double> > wealth(int N, std::vector< std::vector<double> > prm_theta, std::vector< std::vector<double> > prm_M, std::vector< std::vector<double> > value) {
    ...
    for(int i = 0;i <= N; i++){
        wealth_res[i].resize(std::pow(2, i));
        for(int j = 0; j < std::pow(2, i); j++){
...
std::vector< std::vector<double> > utility(int N, std::vector< std::vector<double> > prm_theta, std::vector< std::vector<double> > value) {
    ...
    for(int i = 0;i <= N; i++){
        utility_res[i].resize(std::pow(2, i));
        for(int j = 0; j < std::pow(2, i); j++){
...
std::vector< std::vector<double> > exp_utility(int N, std::vector< std::vector<int> > up_times, std::vector< std::vector<int> > down_times, std::vector< std::vector<double> > prob, std::vector< std::vector<double> >utility) {
    ...
    for(int i = 0;i <= N; i++){
        exp_utility_res[i].resize(std::pow(2, i));
        for(int j = 0; j < std::pow(2, i); j++){
              //if(up_times[i][j] != 0 && down_times[i][j] != 0){
                //exp_utility_res[i][j] = comb(std::pow(2, i)-1, up_times[i][j])*std::pow(prob[i][j], up_times[i][j])*std::pow(1-prob[i][j], down_times[i][j])*utility[i][j];
                exp_utility_res[i][j] = std::pow(prob[i][j], up_times[i][j])*std::pow(1-prob[i][j], down_times[i][j])*utility[i][j];
            /*}else{
                if (down_times[i][j] == 0){
...
std::vector< std::vector<double> > Wn(int N, std::vector< std::vector<int> >up_times, std::vector< std::vector<int> >down_times, std::vector< std::vector<double> >prob, std::vector< std::vector<double> >wealth) {
    ...
    for(int i = 0;i <= N; i++){
        Wn_res[i].resize(std::pow(2, i));
        for(int j = 0; j < std::pow(2, i); j++){
            if(up_times[i][j] != 0 && down_times[i][j] != 0){
                Wn_res[i][j] = comb(std::pow(2, i)-1, up_times[i][j])*std::pow(prob[i][j], up_times[i][j])*std::pow(1-prob[i][j], down_times[i][j])*wealth[i][j];
            }else{
                if(down_times[i][j] == 0){
...
std::vector< double > default_arr(int rep, int default_value){
        std::vector< double > default_arr_res( rep);
        for(int i = 0; i <= rep; i++){
        default_arr_res[i] = default_value*i;
        }
        return default_arr_res; 
}

/*
std::vector< std::vector<double> > for_graph(int N, std::vector< double > default_arr, std::vector< std::vector<int> >up_times, std::vector< std::vector<int> >down_times, std::vector< std::vector<double> >prob) {
        std::vector< std::vector<double> plot_arr( N+1);
        for(int i = 0;i <= N; i++){
         exp_utility_res[i].resize(std::pow(2, i));
        for(int j = 0; j < std::pow(2, i); j++){
                exp_utility_res[i][j] = std::pow(prob[i][j], up_times[i][j])*std::pow(1-prob[i][j], down_times[i][j])*utility[i][j];
